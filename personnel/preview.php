...
?>
    ...
    <div class="form-group col-md-12 preview" id="preview-div">
        <form class="form row" id="" method="POST" action="backend.php" enctype="multipart/form-data">
            <div class="col-md-12" style="background-color:#ecedeb; color:#226305;">
                <span><h4>Basic Information</h5></span>
            </div>
               ...
               <div class="col-md-4">
                ...
                <div class="col-md-12 mb-3" style="background-color:#ecedeb; color:#226305;">
                    <span><h5>Current Posting</h5></span>
                </div>
                <div class="col-md-12">
                        <table  class="table table-striped dt-responsive nowrap w-100 table-sm">
                             <thead>
                                        <tr>
                                            <th>Rank</th>
                                            <th>Date Posted</th>
                                            ...
                                            <th>Designation</th>
                                            <th>Duration</th>
                                            <th>Date Posted Out</th>
                                            <th>Status</th>
                                        </tr>
                                    </thead>
                            <?php 
                                    $query = DB::query("SELECT * FROM postings WHERE  svn='$svn' ORDER BY  posting_id DESC LIMIT 4");
                                    if($query){
                                        $i=1;
                                        foreach($query as $result){
                                            $getRankDetail = getRankDetail($result['rank_id']);
                                            $rankCode = $getRankDetail['rank_code'];
                                            $getCommand = getCommand($result['command']);
                                            $command = $getCommand['command_name'];
                                            $unitId = $result['unit'];
                                            if($unitId=='0'){
                                                $unit='N/A';
                                            }else{
                                                $getUnit = getUnit($unitId);
                                                $unit = $getUnit['unit_name'];
                                            }
                                            $getDesignation = getDesignation($result['designation']);
                                            $designation = $getDesignation['designation_name'];
                                            $datePostedOut = $result['date_posted_out'];
                                            if($datePostedOut==''){
                                                $duration = 'N/A';
                                            }else{
                                                $dateIn = strtotime($result['effective_date']);
                                                $dateOut = strtotime($result['date_posted_out']);
                                                 $differenceInSeconds = $dateOut - $dateIn;
                                                if($differenceInSeconds <= 2678400){
                                                    $duration = floor($differenceInSeconds / (60 * 60 * 24)). ' Days';
                                                }else{
                                                    $duration = floor($differenceInSeconds / (30 * 24 * 60 * 60)). ' Months';
                                                }
                                ...
                                ?>
                                        ...
                                        <tr class="">
                                            ...
                                            <td><?php echo $result['date_posted_out']; ?></td>
